...
export default function CardSlider({
  ...
  const previous = (behavior = "smooth", step = 1) =>
    scrollToIndex(currentIndex - step, behavior);
  const next = (behavior = "smooth", step = 1) =>
    scrollToIndex(currentIndex + step, behavior);
  ...
  const onPointerUp = () => {
    ...
    const effThreshold = isDesktop
      ? Math.max(8, Math.floor(dragThreshold * 0.75))
      : dragThreshold;
    ...
    <div className="w-full">
      <div className="relative">
        <div
          ...
          onMouseLeave={() => {
            onPointerUp();
            if (pauseOnHover && !isAutoPlayPaused) startAutoPlay();
          }}
        >
          {items?.map((item, idx) => (
            <div
